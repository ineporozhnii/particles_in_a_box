...
def main():
   ...
   # plt.style.use("dark_background")
    ...
    velocity_limits_x = 2*position_limits_x
    velocity_limits_y = 2*position_limits_y
    ...
    if compare:
        restitution_coef_bc_list = [1, 0.8, 0.4]
        restitution_coef_pc_list = [1, 0.8, 0.4]
        ...
        simulation = FuncAnimation(fig, utils.simulate_compare, fargs=([particles_1, particles_2, particles_3], acceleration_vector_list, [ax1, ax2, ax3], box_size_x, box_size_y, restitution_coef_bc_list, restitution_coef_pc_list), frames=n_frames, interval=1, repeat=False)
    else: 
    ...
    if save:
        ...
        simulation.save('simulation_rest_coef_comp_2.mp4', writer=writer)
    else:
        plt.show()
